...
# Stage5
```{r stage5-split-sleep-model}
# Stage 5: Split the averaged sleep variable and fit the robust model
library(MASS)
library(car)
library(dplyr)

# 1. Create the new base formula by replacing Avg_sleep_hours with SLD012 and SLD013
# We extract the main effects formula, remove Avg_sleep_hours, and insert the two split variables.
main_effects_formula <- "Avg_sleep_hours + Insomnia + Sleep_apnea +
                         Age_in_years_at_screening + Sex + Race + 
                         Family_income_to_poverty_ratio + Body_mass_index +
                         ...
                         Seen_talked_to_a_mental_health_professional"
split_main_effects <- gsub("Avg_sleep_hours \\+ ", "", main_effects_formula)
split_main_effects <- paste("SLD012 + SLD013 +", split_main_effects)

# 2. Re-create the full formula string with split sleep duration
split_formula_string <- paste("PHQ9_total ~", split_main_effects)
if (length(interaction_terms) > 0) {
  # Note: Existing interaction terms involving Avg_sleep_hours will be excluded here
  # to keep the formula simple. If interactions with SLD012/SLD013 were desired, 
  # the interaction_terms list would need to be updated.
  split_formula_string <- paste(split_formula_string, "+", interaction_string)
}

# Fit the new robust model
model_split_sleep <- glm.nb(as.formula(split_formula_string), data = data_analysis)

cat("=== Stage 5: Split Sleep Duration Model Summary (NB-GLM) ===\n\n")
summary(model_split_sleep)
```
