...
if (!dir.exists("figures")) {
  ...
  cat("Figures directory already exists\n")
}
```
...
# Convert lifestyle variables to binary/categorical (Stage 2 covariates)
# These are typically coded as: 1 = Yes, 2 = No, or ordinal scales 0, 1, 2
# We convert to binary: 1 = Yes/Present, 0 = No/Absent
data$Work_vigorous_intensity_activity <- ifelse(data$Work_vigorous_intensity_activity == 1, 1, 0)
data$Work_moderate_intensity_activity <- ifelse(data$Work_moderate_intensity_activity == 1, 1, 0)
data$Transport_walk_or_bicycle <- ifelse(data$Transport_walk_or_bicycle == 1, 1, 0)
data$Recreation_vigorous_intensity_activity <- ifelse(data$Recreation_vigorous_intensity_activity == 1, 1, 0)
# Secondhand_smoke_exposure_at_home is already 0/1 binary

# Convert medical intervention variables to binary (Stage 3 covariates)
# Took_any_prescription_medicine: 1 = Yes, 2 = No -> convert to 1 = Yes, 0 = No
data$Took_any_prescription_medicine <- ifelse(data$Took_any_prescription_medicine == 1, 1, 0)

# Seen_talked_to_a_mental_health_professional: 1 = Yes, 2 = No -> convert to 1 = Yes, 0 = No
data$Seen_talked_to_a_mental_health_professional <- ifelse(data$Seen_talked_to_a_mental_health_professional == 1, 1, 0)

# Number_of_prescription_medicines is a count variable, keep as numeric
```
...
# Export figure
png("figures/phq9_distribution.png", width = 1200, height = 600)
par(mfrow = c(1, 2))
hist(data_analysis$PHQ9_total,
     breaks = 28,
     main = "Distribution of PHQ-9 Total Score",
...
hist(data_analysis$Avg_sleep_hours,
     ...
     border = "white")
dev.off()
cat("Figure saved: figures/phq9_distribution.png\n")
```
...
# Export figure
png("figures/sleep_phq9_loess.png", width = 1200, height = 600)
par(mfrow = c(1, 2))
plot(data_analysis$Avg_sleep_hours, data_analysis$PHQ9_total,
     pch = 16, col = rgb(0, 0, 0, 0.1),
     xlab = "Average Sleep Hours",
     ylab = "PHQ-9 Total Score",
     main = "Sleep Duration vs Depression Severity")
loess_fit2 <- loess(PHQ9_total ~ Avg_sleep_hours, data = data_analysis)
lines(sleep_seq, predict(loess_fit2, data.frame(Avg_sleep_hours = sleep_seq)),
      col = "red", lwd = 2)
boxplot(PHQ9_total ~ Insomnia, data = data_analysis,
        names = c("No Insomnia", "Insomnia"),
        xlab = "Insomnia Status",
        ...
        col = c("lightblue", "salmon"))
dev.off()
cat("Figure saved: figures/sleep_phq9_loess.png\n")
```
...
plot(simulationOutput, asFactor = FALSE)
...
# Export DHARMa diagnostics
png("figures/dharma_diagnostics.png", width = 1200, height = 600)
par(mfrow = c(1, 2))
plot(simulationOutput, asFactor = FALSE)
dev.off()
cat("Figure saved: figures/dharma_diagnostics.png\n")
```
  ...
  for (k in 1:n_sig) {
    ...
    if (length(lifestyle_levels) <= 5) {
      ...
      for (m in 1:length(lifestyle_levels)) {
        ...
        # Create base data frame with reference values
        # For binary variables (0/1), use reference level (0)
        # For continuous variables, use mean
        new_data <- data.frame(
          Avg_sleep_hours = mean(data_analysis$Avg_sleep_hours, na.rm = TRUE),
          Insomnia = 0,  # Reference level for binary
          Sleep_apnea = 0,  # Reference level for binary
          ...
          Work_vigorous_intensity_activity = 0,  # Reference level for binary
          Work_moderate_intensity_activity = 0,  # Reference level for binary
          Transport_walk_or_bicycle = 0,  # Reference level for binary
          Recreation_vigorous_intensity_activity = 0,  # Reference level for binary
          Secondhand_smoke_exposure_at_home = 0  # Reference level for binary
        )
    ...
    if (length(medical_levels) <= 5) {
      ...
      for (m in 1:length(medical_levels)) {
        ...
        new_data <- data.frame(
          ...
          Recreation_vigorous_intensity_activity = 0,  # Reference level for binary
          Secondhand_smoke_exposure_at_home = 0,  # Reference level for binary
          Took_any_prescription_medicine = 0,  # Reference level for binary
          Number_of_prescription_medicines = mean(data_analysis$Number_of_prescription_medicines, na.rm = TRUE),
          Seen_talked_to_a_mental_health_professional = 0  # Reference level for binary
        )
...
# Export Cook's Distance plot for Stage 3
png("figures/cook_distance_stage3.png", width = 800, height = 600)
plot(model_medical_final, which = 4, main = "Cook's Distance (Stage 3 GLM.NB)")
abline(h = 4/nrow(data_analysis), col = "red", lty = 2, lwd = 2)
text(x = par("usr")[2] * 0.7, y = 4/nrow(data_analysis) * 1.3,
     labels = paste("Threshold: 4/n =", round(4/nrow(data_analysis), 6)),
     col = "red", pos = 3)
dev.off()
cat("Figure saved: figures/cook_distance_stage3.png\n")
```

Robustness Check: Model Without Influential Points
```{r}
# ========================================
# Stage 3: Robustness Check - Removing Influential Points
# ========================================

cat("\n=== Stage 3 Robustness Test: Model Without Influential Points ===\n\n")

# 1. Identify influential points using Cook's D > 4/n threshold
cooks_d_s3 <- cooks.distance(model_medical_final)
n_s3 <- nrow(data_analysis)
threshold_s3 <- 4/n_s3

influential_idx_s3 <- which(cooks_d_s3 > threshold_s3)
cat("Cook's Distance threshold (4/n):", round(threshold_s3, 6), "\n")
cat("Number of influential points identified:", length(influential_idx_s3), "\n")
cat("Percentage of data:", round(length(influential_idx_s3) / n_s3 * 100, 2), "%\n\n")

# Display some information about influential points
if (length(influential_idx_s3) > 0) {
  cat("Observation indices of influential points (first 20):\n")
  print(head(influential_idx_s3, 20))

  cat("\nCook's Distance values for most influential points:\n")
  top_influential_s3 <- sort(cooks_d_s3, decreasing = TRUE)[1:min(10, length(influential_idx_s3))]
  print(round(top_influential_s3, 6))

  # 2. Create dataset without influential points
  data_robust_s3 <- data_analysis[-influential_idx_s3, ]
  cat("\nOriginal sample size:", n_s3, "\n")
  cat("Robust sample size:", nrow(data_robust_s3), "\n\n")

  # 3. Refit Stage 3 final model on cleaned data
  cat("=== Refitting Stage 3 Final Model Without Influential Points ===\n")

  # Use the same formula as model_medical_final
  model_medical_robust <- MASS::glm.nb(formula(model_medical_final), data = data_robust_s3)

  cat("\nRobust Model AIC:", round(AIC(model_medical_robust), 2), "\n")
  cat("Robust Model BIC:", round(BIC(model_medical_robust), 2), "\n\n")

  # 4. Compare coefficients: original vs robust model
  cat("\n=== Comparison: Original vs Robust Model Coefficients ===\n\n")

  # Extract coefficients for sleep variables
  coefs_original_s3 <- summary(model_medical_final)$coefficients
  coefs_robust_s3 <- summary(model_medical_robust)$coefficients

  sleep_vars_s3 <- c("Avg_sleep_hours", "Insomnia", "Sleep_apnea")

  comparison_table_s3 <- data.frame(
    Variable = character(),
  ...
  for (var in sleep_vars_s3) {
    if (var %in% rownames(coefs_original_s3) && var %in% rownames(coefs_robust_s3)) {
      orig_est <- coefs_original_s3[var, "Estimate"]
      robust_est <- coefs_robust_s3[var, "Estimate"]
      diff <- robust_est - orig_est
      pct_change <- (diff / orig_est) * 100

      comparison_table_s3 <- rbind(comparison_table_s3,
                                   data.frame(
                                     Variable = var,
                                     Original_Estimate = orig_est,
                                     Robust_Estimate = robust_est,
                                     Difference = diff,
                                     Pct_Change = pct_change,
                                     Original_pvalue = coefs_original_s3[var, "Pr(>|z|)"],
                                     Robust_pvalue = coefs_robust_s3[var, "Pr(>|z|)"],
                                     stringsAsFactors = FALSE
                                   ))
    }
  }

  cat("Sleep Variable Coefficient Comparison:\n")
  print(comparison_table_s3, digits = 4, row.names = FALSE)

  # 5. Statistical comparison summary
  cat("\n=== Robustness Assessment ===\n")
  max_pct_change <- max(abs(comparison_table_s3$Pct_Change))
  cat("Maximum percentage change in sleep coefficients:", round(max_pct_change, 2), "%\n")

  if (max_pct_change < 10) {
    cat("Conclusion: Model is ROBUST. Key sleep coefficients changed by less than 10%.\n")
  } else if (max_pct_change < 20) {
    cat("Conclusion: Model is MODERATELY ROBUST. Some coefficients changed by 10-20%.\n")
  } else {
    cat("Conclusion: Model shows SENSITIVITY to influential points (>20% change).\n")
  }

  # 6. Compare model fit statistics
  cat("\n=== Model Fit Comparison ===\n")
  cat("Original Model AIC:", round(AIC(model_medical_final), 2), "\n")
  cat("Robust Model AIC:", round(AIC(model_medical_robust), 2), "\n")
  cat("AIC Change:", round(AIC(model_medical_robust) - AIC(model_medical_final), 2), "\n\n")
  cat("Original Model BIC:", round(BIC(model_medical_final), 2), "\n")
  cat("Robust Model BIC:", round(BIC(model_medical_robust), 2), "\n")
  cat("BIC Change:", round(BIC(model_medical_robust) - BIC(model_medical_final), 2), "\n")

} else {
  cat("No influential points detected with Cook's D > 4/n threshold.\n")
}
```
...
# Stage4
```{r stage4-individual-symptoms}
# Stage 4: Analyze the impact of sleep quality variables on individual PHQ-9 
# Define the sleep quality variables to be jointly tested
sleep_predictors <- c("Avg_sleep_hours", "Insomnia", "Sleep_apnea")

# Data frame to store results
stage4_results <- data.frame(
  Symptom = character(),
...
cat("=== Stage 4: Joint Significance Test (Wald Chi-Sq) for Sleep Predictors on PHQ-9 Items ===\n\n")

# Use the base formula from the Stage 3 Final Model, replacing the outcome
base_predictor_formula <- gsub("PHQ9_total ~ ", "", final_formula_s3)

for (item in phq9_items) {
  
  # Create formula for the current symptom item
  item_formula_str <- paste(item, "~", base_predictor_formula)
  
  # Fit Negative Binomial GLM (assuming consistency with total score model)
  model_item <- tryCatch({
    MASS::glm.nb(as.formula(item_formula_str), data = data_analysis)
  }, error = function(e) {
    cat(paste("Error fitting model for", item, ":", e$message, "\n"))
    return(NULL)
  })
  
  if (!is.null(model_item)) {
    # Perform joint significance test (Wald Chi-Sq test) for the three sleep predictors
    # H0: All coefficients for Avg_sleep_hours, Insomnia, and Sleep_apnea are simultaneously zero.
    test_result <- car::linearHypothesis(model_item, hypothesis.matrix = sleep_predictors)
    
    # Extract results
    chisq_stat <- test_result$`Chisq`[2]
    df <- test_result$`Df`[2]
    p_val <- test_result$`Pr(>Chisq)`[2]
    ...
    # Store results
    stage4_results <- rbind(stage4_results,
                            data.frame(
                              Symptom = item,
...
print(stage4_results, row.names = FALSE, digits = 4)
```
# stage5
```{r}
# ==========================================
# Stage 5: In-Depth Sleep Predictors
# ==========================================

# 1. Prepare variables for Stage 5
# Instead of Avg_sleep_hours, we use weekday and weekend sleep separately
# Note: Variables names from dataset2_processed_zero.csv
data_analysis$Weekday_sleep <- data_analysis$Hours_of_sleep_on_weekdays_workdays
data_analysis$Weekend_sleep <- data_analysis$Hours_of_sleep_on_weekends_non_workdays

# 2. Fit the model with separate sleep variables (using Negative Binomial GLM)
# We keep the covariates from the robust model structure
stage5_formula_str <- "PHQ9_total ~ Weekday_sleep + Weekend_sleep + Insomnia + Sleep_apnea + 
                       Sex + Race + Age_in_years_at_screening + 
                       Family_income_to_poverty_ratio + Body_mass_index + 
                       Work_vigorous_intensity_activity + Work_moderate_intensity_activity + 
                       Transport_walk_or_bicycle + Recreation_vigorous_intensity_activity + 
                       Secondhand_smoke_exposure_at_home"

model_stage5 <- MASS::glm.nb(as.formula(stage5_formula_str), data = data_analysis)

cat("=== Stage 5: Model with Separate Weekday and Weekend Sleep ===\n\n")
summary(model_stage5)

# 3. Hypothesis Test: Are the coefficients for Weekday and Weekend sleep equal?
# H0: beta_weekday = beta_weekend
cat("\n=== Testing Equality of Weekday and Weekend Sleep Coefficients ===\n")
test_equality <- car::linearHypothesis(model_stage5, "Weekday_sleep = Weekend_sleep")
print(test_equality)
```

```{r}
# 4. Compare Standardized Coefficients for Sleep Predictors
# To compare the relative importance, we standardize continuous predictors
data_scaled <- data_analysis %>%
  mutate(across(c(Weekday_sleep, Weekend_sleep, Age_in_years_at_screening, 
                  Family_income_to_poverty_ratio, Body_mass_index), 
                ~ as.vector(scale(.))))

model_stage5_std <- MASS::glm.nb(as.formula(stage5_formula_str), data = data_scaled)

cat("\n=== Standardized Coefficients for Sleep Predictors ===\n")
std_coefs <- summary(model_stage5_std)$coefficients
sleep_preds <- c("Weekday_sleep", "Weekend_sleep", "Insomnia", "Sleep_apnea")
print(std_coefs[rownames(std_coefs) %in% sleep_preds, ])

# 5. Outlier and Influential Point Detection for Stage 5 Model
cat("\n=== Influence Diagnostics for Stage 5 Model ===\n")
cooks_d_s5 <- cooks.distance(model_stage5)
n_s5 <- nrow(data_analysis)
p_s5 <- length(coef(model_stage5))

# Identify points with high Cook's distance
influential_s5 <- which(cooks_d_s5 > (4/n_s5)) 
cat("Number of points with Cook's D > 4/n:", length(influential_s5), "\n")

# Visualization of Cook's Distance
plot(cooks_d_s5, type = "h", main = "Cook's Distance (Stage 5 Model)",
     ylab = "Cook's distance", xlab = "Index")
abline(h = 4/n_s5, col = "red", lty = 2)

# Export Cook's Distance plot for Stage 5
png("figures/cook_distance_stage5.png", width = 800, height = 600)
plot(cooks_d_s5, type = "h", main = "Cook's Distance (Stage 5 Model)",
     ylab = "Cook's distance", xlab = "Observation Index")
abline(h = 4/n_s5, col = "red", lty = 2, lwd = 2)
text(x = length(cooks_d_s5) * 0.7, y = 4/n_s5 * 1.5,
     labels = paste("Threshold: 4/n =", round(4/n_s5, 6)), col = "red", pos = 3)
dev.off()
cat("Figure saved: figures/cook_distance_stage5.png\n")

# 6. Model Comparison: Averaged Sleep (Stage 2) vs. Separate Sleep (Stage 5)
# Using AIC to see if splitting sleep duration improves model fit
cat("\n=== Model Comparison (AIC) ===\n")
cat("Stage 2 Model (Averaged Sleep) AIC:", round(AIC(model_lifestyle), 2), "\n")
cat("Stage 5 Model (Separate Sleep) AIC:", round(AIC(model_stage5), 2), "\n")
```

```{r}
# ========================================
# Stage 5: Robustness Check - Removing Influential Points
# ========================================

cat("\n=== Stage 5 Robustness Test: Model Without Influential Points ===\n\n")

# 1. Identify influential points using Cook's D > 4/n threshold
threshold_s5 <- 4/n_s5

cat("Cook's Distance threshold (4/n):", round(threshold_s5, 6), "\n")
cat("Number of influential points identified:", length(influential_s5), "\n")
cat("Percentage of data:", round(length(influential_s5) / n_s5 * 100, 2), "%\n\n")

# Display information about influential points
if (length(influential_s5) > 0) {
  cat("Observation indices of influential points (first 20):\n")
  print(head(influential_s5, 20))

  cat("\nCook's Distance values for most influential points:\n")
  top_influential_s5 <- sort(cooks_d_s5, decreasing = TRUE)[1:min(10, length(influential_s5))]
  print(round(top_influential_s5, 6))

  # 2. Create dataset without influential points
  data_robust_s5 <- data_analysis[-influential_s5, ]
  cat("\nOriginal sample size:", n_s5, "\n")
  cat("Robust sample size:", nrow(data_robust_s5), "\n\n")

  # 3. Refit Stage 5 model on cleaned data
  cat("=== Refitting Stage 5 Model Without Influential Points ===\n")

  model_stage5_robust <- MASS::glm.nb(formula(model_stage5), data = data_robust_s5)

  cat("\nRobust Model AIC:", round(AIC(model_stage5_robust), 2), "\n")
  cat("Robust Model BIC:", round(BIC(model_stage5_robust), 2), "\n\n")

  # 4. Compare weekday/weekend sleep coefficients: original vs robust
  cat("\n=== Comparison: Original vs Robust Model Coefficients ===\n\n")

  coefs_original_s5 <- summary(model_stage5)$coefficients
  coefs_robust_s5 <- summary(model_stage5_robust)$coefficients

  sleep_vars_s5 <- c("Weekday_sleep", "Weekend_sleep", "Insomnia", "Sleep_apnea")

  comparison_table_s5 <- data.frame(
    Variable = character(),
    Original_Estimate = numeric(),
    Robust_Estimate = numeric(),
    Difference = numeric(),
  ...
  for (var in sleep_vars_s5) {
    if (var %in% rownames(coefs_original_s5) && var %in% rownames(coefs_robust_s5)) {
      orig_est <- coefs_original_s5[var, "Estimate"]
      robust_est <- coefs_robust_s5[var, "Estimate"]
      diff <- robust_est - orig_est
      pct_change <- (diff / orig_est) * 100

      comparison_table_s5 <- rbind(comparison_table_s5,
                                   data.frame(
                                     Variable = var,
                                     Original_Estimate = orig_est,
                                     Robust_Estimate = robust_est,
                                     Difference = diff,
                                     Pct_Change = pct_change,
                                     Original_pvalue = coefs_original_s5[var, "Pr(>|z|)"],
                                     Robust_pvalue = coefs_robust_s5[var, "Pr(>|z|)"],
                                     stringsAsFactors = FALSE
                                   ))
    }
  }
  ...
  cat("Sleep Variable Coefficient Comparison:\n")
  print(comparison_table_s5, digits = 4, row.names = FALSE)

  # 5. Re-test equality hypothesis on robust model
  cat("\n=== Re-Testing Weekday vs Weekend Sleep Equality (Robust Model) ===\n")
  test_equality_robust <- car::linearHypothesis(model_stage5_robust, "Weekday_sleep = Weekend_sleep")
  print(test_equality_robust)

  # Compare with original test
  test_equality_original <- car::linearHypothesis(model_stage5, "Weekday_sleep = Weekend_sleep")

  cat("\nComparison of Equality Tests:\n")
  cat("Original model p-value:", format(test_equality_original$`Pr(>Chisq)`[2], digits = 4), "\n")
  cat("Robust model p-value:", format(test_equality_robust$`Pr(>Chisq)`[2], digits = 4), "\n")

  # 6. Statistical comparison summary
  cat("\n=== Robustness Assessment ===\n")
  max_pct_change <- max(abs(comparison_table_s5$Pct_Change))
  cat("Maximum percentage change in sleep coefficients:", round(max_pct_change, 2), "%\n")
  ...
  # 7. Compare model fit statistics
  cat("\n=== Model Fit Comparison ===\n")
  cat("Original Model AIC:", round(AIC(model_stage5), 2), "\n")
  cat("Robust Model AIC:", round(AIC(model_stage5_robust), 2), "\n")
  cat("AIC Change:", round(AIC(model_stage5_robust) - AIC(model_stage5), 2), "\n\n")
  cat("Original Model BIC:", round(BIC(model_stage5), 2), "\n")
  cat("Robust Model BIC:", round(BIC(model_stage5_robust), 2), "\n")
  cat("BIC Change:", round(BIC(model_stage5_robust) - BIC(model_stage5), 2), "\n")
