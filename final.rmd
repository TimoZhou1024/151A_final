...
Stage 1 establishes the base model with main effects only (no interactions are tested at this stage).

```{r final-model-s1}
cat("=== Final Stage 1 Model (Base Model) ===\n\n")

cat("Model Formula:\n")
cat("PHQ9_total ~ Avg_sleep_hours + Insomnia + Sleep_apnea +\n")
cat("             Sex + Race + Age_in_years_at_screening +\n")
cat("             Family_income_to_poverty_ratio + Body_mass_index\n\n")

# The final Stage 1 model is the base model
model_base_final <- model_base

cat("=== Key Statistics ===\n")
cat("R-squared:", round(summary(model_base_final)$r.squared, 4), "\n")
cat("Adjusted R-squared:", round(summary(model_base_final)$adj.r.squared, 4), "\n")
cat("Residual standard error:", round(summary(model_base_final)$sigma, 4), "\n")
cat("AIC:", round(AIC(model_base_final), 2), "\n")
cat("BIC:", round(BIC(model_base_final), 2), "\n")

cat("\n=== Sleep Variable Coefficients ===\n\n")
...
cat("\n=== Interpretation ===\n")
cat("This base model establishes the fundamental relationships between sleep\n")
cat("variables and depression severity, controlling for basic demographics.\n")
cat("Subsequent stages will build upon this foundation by adding lifestyle\n")
cat("and medical intervention variables.\n")
```
...
Based on the interaction tests above, we now construct the final Stage 2 model by including only significant interactions (if any).

```{r final-model-s2}
cat("=== Final Stage 2 Model Construction ===\n\n")
...
if (nrow(significant_interactions) > 0) {
  ...
  model_lifestyle_final <- lm(as.formula(final_formula_s2), data = data_analysis)

  cat("\n=== Final Stage 2 Model (with significant interactions) ===\n\n")
  ...
  cat("=== Final Stage 2 Model (main effects only) ===\n\n")
...
cat("R-squared:", round(summary(model_lifestyle_final)$r.squared, 4), "\n")
cat("Adjusted R-squared:", round(summary(model_lifestyle_final)$adj.r.squared, 4), "\n")
cat("Residual standard error:", round(summary(model_lifestyle_final)$sigma, 4), "\n")
...
## Final Model for Stage 3

Based on the interaction tests above, we now construct the final Stage 3 model by including only significant interactions (if any).

```{r final-model-s3}
cat("=== Final Stage 3 Model Construction ===\n\n")
...
if (nrow(significant_interactions_s3) > 0) {
  ...
  final_formula_s3 <- paste(
    ...
    "Number_of_prescription_medicines +",
    "Seen_talked_to_a_mental_health_professional +",
    paste(interaction_terms_s3, collapse = " + ")
  )

  model_medical_final <- lm(as.formula(final_formula_s3), data = data_analysis)

  cat("\n=== Final Stage 3 Model (with significant interactions) ===\n\n")
  ...
  cat("=== Final Stage 3 Model (main effects only) ===\n\n")
  cat("Formula: PHQ9_total ~ Avg_sleep_hours + Insomnia + Sleep_apnea +\n")
  cat("         Sex + Race + Age_in_years_at_screening +\n")
  cat("         Family_income_to_poverty_ratio + Body_mass_index +\n")
  cat("         Work_vigorous_intensity_activity + Work_moderate_intensity_activity +\n")
...
cat("R-squared:", round(summary(model_medical_final)$r.squared, 4), "\n")
cat("Adjusted R-squared:", round(summary(model_medical_final)$adj.r.squared, 4), "\n")
cat("Residual standard error:", round(summary(model_medical_final)$sigma, 4), "\n")
...
cat("\n=== Sleep Variable Coefficients in Final Stage 3 Model ===\n\n")
...
This section compares the **final models** from each stage, including any significant interactions that were identified.
...
model_summary <- data.frame(
  Model = c("Stage 1: Base",
            "Stage 2: + Lifestyle (Final)",
            "Stage 3: + Medical (Final)"),
  R_squared = c(summary(model_base_final)$r.squared,
                summary(model_lifestyle_final)$r.squared,
                summary(model_medical_final)$r.squared),
  Adj_R_squared = c(summary(model_base_final)$adj.r.squared,
                    summary(model_lifestyle_final)$adj.r.squared,
                    summary(model_medical_final)$adj.r.squared),
  ...
  Residual_SE = c(summary(model_base_final)$sigma,
                  summary(model_lifestyle_final)$sigma,
                  summary(model_medical_final)$sigma),
  N_predictors = c(
...
cat("\n=== Sleep Variable Effects Across Final Models ===\n\n")
...
```{r final-conclusions}
cat("\n=== KEY CONCLUSIONS FROM STAGES 1-3 (FINAL MODELS) ===\n\n")

final_coefs <- summary(model_medical_final)$coefficients

cat("FINAL STAGE 3 MODEL:\n")
cat("This model includes demographics, lifestyle factors, medical interventions,\n")
cat("and any significant interaction terms identified during testing.\n\n")

cat("1. SLEEP DURATION:\n")
cat("   After full adjustment, each additional hour of sleep is associated with\n")
...
cat("   - Stage 1 (Base) R² =", round(summary(model_base_final)$r.squared, 4), "\n")
cat("   - Stage 2 (+ Lifestyle + interactions) R² =",
    round(summary(model_lifestyle_final)$r.squared, 4), "\n")
cat("   - Stage 3 (+ Medical + interactions) R² =",
    round(summary(model_medical_final)$r.squared, 4), "\n\n")
